...
def transcode(flac_file, output_dir, output_format):
    ...
    tagged.copy_tags(flac_file, transcode_file)
    (ok, msg) = tagged.check_tags(transcode_file)
    if not ok:
...
def make_torrent(input_dir, output_dir, tracker, passkey, piece_length, source):
    if source == 'OPS':
    ...
    command = ["mktorrent", "-p", "-s", source, "-a", tracker_url, "-o", torrent, "-l", piece_length, input_dir]
